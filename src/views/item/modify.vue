...
// 将chars转义成html
import { charsToHtml } from '@/utils/replace'
...
export default {
  ...
  created() {
    // 获取参数ID
    this.formData.id = Number(this.$router.history.current.params.id)
    // 获取商品详情
    this.getDetail()
  },
  methods: {
    // 获取商品详情
    getDetail() {
      const param = { url: '/api/pro/item/detail', data: { id: this.formData.id }}
      ajax.post(param).then((data) => {
        if (data.success === 'true') {
          this.form = data.data
          // 设置子组件的图片url
          this.$refs.uploadImage.url = this.form.image
          // 设置子组件ckeditor的内容
          this.$refs.ckeditor.editorData = charsToHtml(this.form.content)
        } else {
    ...
    add() {
      const param = { url: '/api/pro/item/modify', data: this.formData }
      ajax.post(param).then((data) => {
        if (data.success === 'true') {
